function initJotFormAutofill() {
 ...
 // Modify the displaySubmissions function
    ...
    if (ratingSelect) {
        // Set the rating to 5 (highest) by default
        ratingSelect.value = '5';
        ratingSelect.dispatchEvent(new Event('change', { bubbles: true }));

        // If you want to use the value from the submission (if available), use this code instead:
        // const ratingValue = safeGet(submission, 'answers.187.answer');
        // if (ratingValue && ratingSelect.querySelector(`option[value="${ratingValue}"]`)) {
        //     ratingSelect.value = ratingValue;
        // } else {
        //     ratingSelect.value = '5'; // Default to 5 if no valid value in submission
        // }
        // ratingSelect.dispatchEvent(new Event('change', { bubbles: true }));
    } else {
        console.warn('Rating select not found');
    }
